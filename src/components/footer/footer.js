    ...
    (() => {
        if ($.__config.links.footer.length > 0) {
            ...
            for (let i = 0; i < $.__config.links.footer.length; i++) {
                linksHtml +=
                    '<a href="' +
                    $.__config.links.footer[i][1] +
                    '" target="_blank">' +
                    $.__config.links.footer[i][0] +
                    '</a>';
        ...
        switch (parseInt(config.style)) {
            case 1:
                $('#footer')
                    ...
                    .css(
                        'background',
                        'url(//www.wangyangyang.vip/usr/uploads/imgs/o_221114131838_footer.webp)  no-repeat 50%'
                    );
                break;
            case 2:
            default:
                ...
                footerStyle2
                    ...
                    .css(
                        'background',
                        'url(//www.wangyangyang.vip/usr/uploads/imgs/o_221114132857_clouds.webp)  repeat-x'
                    );
                footerStyle2
                    .find('.background')
                    .css(
                        'background',
                        'url(//www.wangyangyang.vip/usr/uploads/imgs/o_221114134558_background.webp)  repeat-x'
                    );
                footerStyle2
                    .find('.foreground')
                    .css(
                        'background',
                        'url(//www.wangyangyang.vip/usr/uploads/imgs/o_221114132230_foreground.webp)  repeat-x'
                    );
                break;
        }
    })();
        ...
        if ($.__config.umami?.url && $.__config.umami?.shareId) {
            ...
            $.__timeIds.umamiTId = window.setInterval(() => {
                request(`${baseUrl}/api/share/${$.__config.umami.shareId}`).then((r) => {
                    Promise.all([
                        request(
                            `${baseUrl}/api/websites/${
                                r.id
                            }/stats?start_at=${$.__tools.getTodayStart()}&end_at=${$.__tools.getTodayEnd()}`,
                            'GET',
                            {},
                            { 'x-umami-share-token': r.token }
                        ),
                        request(
                            `${baseUrl}/api/websites/${
                                r.id
                            }/stats?start_at=${$.__tools.getYesterdayStart()}&end_at=${$.__tools.getYesterdayEnd()}`,
                            'GET',
                            {},
                            { 'x-umami-share-token': r.token }
                        ),
                        request(
                            `${baseUrl}/api/websites/${r.id}/active`,
                            'GET',
                            {},
                            { 'x-umami-share-token': r.token }
                        ),
