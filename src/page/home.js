...
export default function main() {
    ...
    (() => {
        ...
        function topTitleContent(r) {
            if (r?.status === 'success') {
                ...
                let content =
                    r?.content || `《${r.data.origin.title}》 - ${r.data.origin.dynasty} - ${r.data.origin.author}`;
        ...
        if ($.__config.banner.home.titleSource === 'one')
            request('https://api.wangyangyang.vip/').then((r) => topTitleContent(r));
        if ($.__config.banner.home.titleSource === 'jinrishici')
            request('https://v2.jinrishici.com/one.json').then((r) => topTitleContent(r));
    })();
     ...
     */
    (() => {
        let read = $('#main .c_b_p_desc_readmore'),
            titleList = $('#main .postTitle');
        read.text('阅读全文 »');
        $.each(titleList, (i) => {
        ...
        function postMetaHtml(postDescText) {
            ...
            return (
                '<span class="postMeta"><i class="iconfont icon-schedule"></i>发表于 ' +
                info.date +
                '' +
                '<i class="iconfont icon-browse"></i>阅读：' +
                info.vnum +
                '' +
    ...
    (() => {
        ...
        $.each(desc, (i) => {
            ...
            if (img.length > 0) {
                ...
                let html =
                    '<div class="c_b_p_desc_img"><div style="background: url(\'' +
                    src +
                    '\') center center / contain no-repeat;"></div></div>';
                obj.after(html);
            }
        });
    })();
