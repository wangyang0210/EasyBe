'use strict';
(self.webpackChunkeasybe = self.webpackChunkeasybe || []).push([
    [910],
    {
        4791: function (t, e) {
            e.Z =
                '<div class="container"> <div class="menu-wrap optiscroll" id="menuWrap" style="display:none"> <div class="close-button" id="close-button"> <i class="iconfont icon-close"></i> </div> <div class="sidebar-header"> <i class="iconfont icon-guangbo"></i> <span class="sidebar-title-msg"></span> </div> <div class="sidebar-profile"> <div class="user-pic" id="menuBlogAvatar"></div> <div class="sidebar-userinfo" id="introduce"></div> </div> <div class="sidebar-stats"></div> <div class="sidebar-search"> <div class="sidebar-search-div"> <div class="input-group"> <span id="sb-sidebarSearchBox"></span> <div class="input-group-append"> <span class="input-group-text"> <i class="iconfont icon-sousuo"></i> </span> </div> </div> </div> </div> <div class="customize-nav"></div> <div id="calendar-box"></div> <div class="sidebar-menu" id="customize-sidebar-menu"> <ul></ul> </div> <div class="sidebar-menu"> <ul> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-time-fill"></i> <span class="sidebar-dropdown-title">最新随笔</span> </a> <div class="sidebar-submenu" id="sb-sidebarRecentposts"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-label-fill"></i> <span class="sidebar-dropdown-title">我的标签</span> </a> <div class="sidebar-submenu" id="sb-toptags"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-fenleicur"></i> <span class="sidebar-dropdown-title">随笔分类</span> </a> <div class="sidebar-submenu" id="sb-classify"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-browse-fill"></i> <span class="sidebar-dropdown-title">阅读排行</span> </a> <div class="sidebar-submenu" id="sb-topview"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-like-fill"></i> <span class="sidebar-dropdown-title">推荐排行</span> </a> <div class="sidebar-submenu" id="sb-topDiggPosts"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-comments"></i> <span class="sidebar-dropdown-title">最新评论</span> </a> <div class="sidebar-submenu" id="sb-recentComments"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-yongyan"></i> <span class="sidebar-dropdown-title">评论排行</span> </a> <div class="sidebar-submenu" id="sb-commentsRank"></div> </li> <li class="ng-star-inserted sidebar-dropdown"> <a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box"> <i class="iconfont icon-task-fill"></i> <span class="sidebar-dropdown-title">随笔档案</span> </a> <div class="sidebar-submenu" id="sb-record"></div> </li> </ul> </div> <div class="sidebar-footer"></div> <div class="morph-shape" id="morph-shape" style="display:none" data-morph-open="M-7.312,0H15c0,0,66,113.339,66,399.5C81,664.006,15,800,15,800H-7.312V0z;M-7.312,0H100c0,0,0,113.839,0,400c0,264.506,0,400,0,400H-7.312V0z"> <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 100 800" preserveAspectRatio="none"> <path d="M-7.312,0H0c0,0,0,113.839,0,400c0,264.506,0,400,0,400h-7.312V0z"/> </svg> </div> </div> <button class="menu-button" id="open-button"> MENU </button> <div class="content-wrap" id="content-wrap"></div> </div> ';
        },
        7996: function (t, e) {
            e.Z =
                '<div class="dropdown"> <a id="nav_host" href="" target="_self"> <i class="iconfont icon-home-fill"></i>首页 </a> </div> <div class="dropdown"> <a id="nav_connect" href="" target="_blank"> <i class="iconfont icon-zhifeiji"></i>联系 </a> </div> <div class="dropdown"> <a id="nav_rss" href="" target="_blank"> <i class="iconfont icon-keaide"></i>订阅 </a> </div> <div class="dropdown"> <a id="nav_manage" href="" target="_blank"> <i class="iconfont icon-shezhi-fill"></i>管理 </a> </div> ';
        },
        9986: function (t, e, i) {
            i.d(e, {
                Z: function () {
                    return s;
                },
            });
            function s() {
                $('#sidebar_news').prepend(
                    '<div class="main-header" id="main-header"> <div id="nhBannerAnimation"> <ul class="circles"> <li></li> <li></li> <li></li> <li></li> <li></li> <li></li> <li></li> <li></li> <li></li> <li></li> </ul> </div> <div class="vertical"> <div class="main-header-content inner"> <h1 class="page-title" id="homeTopTitle"><span></span></h1> <h2 class="page-description" id="hitokoto"></h2> <h3 class="page-author" id="hitokotoAuthor"></h3> <h1 class="sb-title" id="sbTitle"> <span id="sbTitleText"></span> <span id="sbTitleConsole"></span> </h1> <p class="article-info" id="articleInfo"></p> </div> </div> <a class="scroll-down" href="javascript:void(0);" data-offset="-45"> <span class="hidden">Scroll Down</span> <i class="scroll-down-icon iconfont icon-fanhui"></i> </a> </div>'
                ),
                    (() => {
                        let t,
                            e,
                            s,
                            n = $('#main-header');
                        'home' === $.__status.pageType
                            ? (t =
                                  $.__config.banner.home.background.length > 0
                                      ? $.__config.banner.home.background
                                      : ['https://www.oyo.cool/usr/uploads/imgs/o_220917053600_wallhaven-6k3oox.webp'])
                            : ((t =
                                  $.__config.banner.article.background.length > 0
                                      ? $.__config.banner.article.background
                                      : ['https://www.oyo.cool/usr/uploads/imgs/o_220917053937_wallhaven-j5mz95.webp']),
                              (s = '40vh'),
                              $('#homeTopTitle').hide(),
                              $('.scroll-down').hide(),
                              $('#home').css('margin-top', '40vh'),
                              $('#cb_post_title_url').addClass('post-del-title')),
                            s && n.css('height', s),
                            $.__config.animate.bannerImages?.enable
                                ? i
                                      .e(315)
                                      .then(i.bind(i, 9830))
                                      .then((e) => {
                                          (0, e.default)(
                                              'main-header',
                                              t,
                                              $.__config.animate.bannerImages.options.itemNum,
                                              $.__config.animate.bannerImages.options.time,
                                              $.__config.animate.bannerImages.options.sort,
                                              $.__config.animate.bannerImages.options.current < 0
                                                  ? $.__tools.randomNum(0, t.length - 1)
                                                  : $.__config.animate.bannerImages.options.current
                                          );
                                      })
                                : ((e = t.length > 1 ? t[$.__tools.randomNum(0, t.length - 1)] : t[0] || ''),
                                  n.css({
                                      background: "#222 url('" + encodeURI(e) + "')  center center no-repeat",
                                      'background-size': 'cover',
                                  })),
                            $.__config.banner.text || $('.main-header-content.inner').addClass('textUnselect');
                    })(),
                    $.__event.scroll.handle.push(() => {
                        let t = $('#open-button');
                        $.__event.scroll.temScroll < $.__event.scroll.docScroll &&
                            $.__event.scroll.homeScroll <= $.__event.scroll.docScroll &&
                            !t.hasClass('menu-button-scroll') &&
                            (t.addClass('menu-button-scroll'), t.text('')),
                            $.__event.scroll.temScroll > $.__event.scroll.docScroll &&
                                $.__event.scroll.homeScroll >= $.__event.scroll.docScroll &&
                                t.hasClass('menu-button-scroll') &&
                                (t.removeClass('menu-button-scroll'), t.text('MENU'));
                    });
            }
        },
        6361: function (t, e, i) {
            i.a(
                t,
                async function (t, s) {
                    try {
                        i.r(e),
                            i.d(e, {
                                default: function () {
                                    return c;
                                },
                            });
                        var n = i(9387),
                            o = i(9986),
                            a = i(434),
                            r = i(2561),
                            l = t([n]);
                        function c() {
                            let t = (0, r.Z)();
                            i.e(14).then(i.bind(i, 1166)),
                                i.e(732).then(i.bind(i, 8860)),
                                t.start(),
                                ('12-13' == $.__tools.getTodayDate() ||
                                    $.__config.memorialDays.includes($.__tools.getTodayDate())) &&
                                    $('html').addClass('htmlGray'),
                                setTimeout(() => {
                                    $.each($.__timeIds, (t) => {
                                        null != $.__timeIds[t] && window.clearInterval($.__timeIds[t]);
                                    });
                                }, 3e4),
                                a.Z.init(),
                                (0, n.Z)(),
                                (0, o.Z)(),
                                '' !== $.__config.fontIconExtend &&
                                    $.__tools.dynamicLoadingCss($.__config.fontIconExtend, 1),
                                t.stop();
                        }
                        (n = (l.then ? (await l)() : l)[0]),
                            await $.__tools
                                .dynamicLoadingJs($.__config.default.jqueryrotate)
                                .catch((t) => console.error('jqueryrotate.js', t)),
                            s();
                    } catch (d) {
                        s(d);
                    }
                },
                1
            );
        },
        2561: function (t, e, i) {
            i.d(e, {
                Z: function () {
                    return l;
                },
            });
            let s = {};
            !(function () {
                let t = (s.util = {}),
                    e = Array.prototype.concat,
                    i = Array.prototype.slice;
                (t.bind = function (t, s) {
                    let n = i.call(arguments, 2);
                    return function () {
                        t.apply(s, e.call(n, i.call(arguments)));
                    };
                }),
                    (t.extend = function (t, e) {
                        for (let i in e) e.hasOwnProperty(i) && (t[i] = e[i]);
                    });
                let n = (s.SpringSystem = function (t) {
                    (this._springRegistry = {}),
                        (this._activeSprings = []),
                        (this.listeners = []),
                        (this._idleSpringIndices = []),
                        (this.looper = t || new l()),
                        (this.looper.springSystem = this);
                });
                t.extend(n.prototype, {
                    _springRegistry: null,
                    _isIdle: !0,
                    _lastTimeMillis: -1,
                    _activeSprings: null,
                    listeners: null,
                    _idleSpringIndices: null,
                    createSpring: function (t, e) {
                        let i;
                        return (
                            (i =
                                void 0 === t || void 0 === e
                                    ? r.DEFAULT_ORIGAMI_SPRING_CONFIG
                                    : r.fromOrigamiTensionAndFriction(t, e)),
                            this.createSpringWithConfig(i)
                        );
                    },
                    createSpringWithBouncinessAndSpeed: function (t, e) {
                        let i;
                        return (
                            (i =
                                void 0 === t || void 0 === e
                                    ? r.DEFAULT_ORIGAMI_SPRING_CONFIG
                                    : r.fromBouncinessAndSpeed(t, e)),
                            this.createSpringWithConfig(i)
                        );
                    },
                    createSpringWithConfig: function (t) {
                        let e = new o(this);
                        return this.registerSpring(e), e.setSpringConfig(t), e;
                    },
                    getIsIdle: function () {
                        return this._isIdle;
                    },
                    registerSpring: function (t) {
                        this._springRegistry[t.getId()] = t;
                    },
                    deregisterSpring: function (t) {
                        p(this._activeSprings, t), delete this._springRegistry[t.getId()];
                    },
                    advance: function (t, e) {
                        for (; this._idleSpringIndices.length > 0; ) this._idleSpringIndices.pop();
                        for (let i = 0, s = this._activeSprings.length; i < s; i++) {
                            let s = this._activeSprings[i];
                            s.systemShouldAdvance()
                                ? s.advance(t / 1e3, e / 1e3)
                                : this._idleSpringIndices.push(this._activeSprings.indexOf(s));
                        }
                        for (; this._idleSpringIndices.length > 0; ) {
                            let t = this._idleSpringIndices.pop();
                            t >= 0 && this._activeSprings.splice(t, 1);
                        }
                    },
                    loop: function (t) {
                        let e;
                        -1 === this._lastTimeMillis && (this._lastTimeMillis = t - 1);
                        let i = t - this._lastTimeMillis;
                        this._lastTimeMillis = t;
                        let s = 0,
                            n = this.listeners.length;
                        for (s = 0; s < n; s++)
                            (e = this.listeners[s]), e.onBeforeIntegrate && e.onBeforeIntegrate(this);
                        for (
                            this.advance(t, i),
                                0 === this._activeSprings.length && ((this._isIdle = !0), (this._lastTimeMillis = -1)),
                                s = 0;
                            s < n;
                            s++
                        )
                            (e = this.listeners[s]), e.onAfterIntegrate && e.onAfterIntegrate(this);
                        this._isIdle || this.looper.run();
                    },
                    activateSpring: function (t) {
                        let e = this._springRegistry[t];
                        -1 == this._activeSprings.indexOf(e) && this._activeSprings.push(e),
                            this.getIsIdle() && ((this._isIdle = !1), this.looper.run());
                    },
                    addListener: function (t) {
                        this.listeners.push(t);
                    },
                    removeListener: function (t) {
                        p(this.listeners, t);
                    },
                    removeAllListeners: function () {
                        this.listeners = [];
                    },
                });
                let o = (s.Spring = function t(e) {
                    (this._id = 's' + t._ID++),
                        (this._springSystem = e),
                        (this.listeners = []),
                        (this._currentState = new a()),
                        (this._previousState = new a()),
                        (this._tempState = new a());
                });
                t.extend(o, { _ID: 0, MAX_DELTA_TIME_SEC: 0.064, SOLVER_TIMESTEP_SEC: 0.001 }),
                    t.extend(o.prototype, {
                        _id: 0,
                        _springConfig: null,
                        _overshootClampingEnabled: !1,
                        _currentState: null,
                        _previousState: null,
                        _tempState: null,
                        _startValue: 0,
                        _endValue: 0,
                        _wasAtRest: !0,
                        _restSpeedThreshold: 0.001,
                        _displacementFromRestThreshold: 0.001,
                        listeners: null,
                        _timeAccumulator: 0,
                        _springSystem: null,
                        destroy: function () {
                            (this.listeners = []), (this.frames = []), this._springSystem.deregisterSpring(this);
                        },
                        getId: function () {
                            return this._id;
                        },
                        setSpringConfig: function (t) {
                            return (this._springConfig = t), this;
                        },
                        setCurrentValue: function (t, e) {
                            return (
                                (this._startValue = t),
                                (this._currentState.position = t),
                                e || this.setAtRest(),
                                this.notifyPositionUpdated(!1, !1),
                                this
                            );
                        },
                        getCurrentValue: function () {
                            return this._currentState.position;
                        },
                        getDisplacementDistanceForState: function (t) {
                            return Math.abs(this._endValue - t.position);
                        },
                        setEndValue: function (t) {
                            if (this._endValue == t && this.isAtRest()) return this;
                            (this._startValue = this.getCurrentValue()),
                                (this._endValue = t),
                                this._springSystem.activateSpring(this.getId());
                            for (let t = 0, e = this.listeners.length; t < e; t++) {
                                let e = this.listeners[t].onSpringEndStateChange;
                                e && e(this);
                            }
                            return this;
                        },
                        setVelocity: function (t) {
                            return (
                                t === this._currentState.velocity ||
                                    ((this._currentState.velocity = t),
                                    this._springSystem.activateSpring(this.getId())),
                                this
                            );
                        },
                        setRestSpeedThreshold: function (t) {
                            return (this._restSpeedThreshold = t), this;
                        },
                        isOvershooting: function () {
                            let t = this._startValue,
                                e = this._endValue;
                            return (
                                this._springConfig.tension > 0 &&
                                ((t < e && this.getCurrentValue() > e) || (t > e && this.getCurrentValue() < e))
                            );
                        },
                        advance: function (t, e) {
                            let i = this.isAtRest();
                            if (i && this._wasAtRest) return;
                            let s = e;
                            e > o.MAX_DELTA_TIME_SEC && (s = o.MAX_DELTA_TIME_SEC), (this._timeAccumulator += s);
                            let n,
                                a,
                                r,
                                l,
                                c,
                                d,
                                h,
                                u,
                                _,
                                p,
                                g = this._springConfig.tension,
                                f = this._springConfig.friction,
                                m = this._currentState.position,
                                v = this._currentState.velocity,
                                b = this._tempState.position,
                                w = this._tempState.velocity;
                            for (; this._timeAccumulator >= o.SOLVER_TIMESTEP_SEC; )
                                (this._timeAccumulator -= o.SOLVER_TIMESTEP_SEC),
                                    this._timeAccumulator < o.SOLVER_TIMESTEP_SEC &&
                                        ((this._previousState.position = m), (this._previousState.velocity = v)),
                                    (n = v),
                                    (a = g * (this._endValue - b) - f * v),
                                    (b = m + n * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (w = v + a * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (r = w),
                                    (l = g * (this._endValue - b) - f * w),
                                    (b = m + r * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (w = v + l * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (c = w),
                                    (d = g * (this._endValue - b) - f * w),
                                    (b = m + c * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (w = v + d * o.SOLVER_TIMESTEP_SEC * 0.5),
                                    (h = w),
                                    (u = g * (this._endValue - b) - f * w),
                                    (_ = (1 / 6) * (n + 2 * (r + c) + h)),
                                    (p = (1 / 6) * (a + 2 * (l + d) + u)),
                                    (m += _ * o.SOLVER_TIMESTEP_SEC),
                                    (v += p * o.SOLVER_TIMESTEP_SEC);
                            (this._tempState.position = b),
                                (this._tempState.velocity = w),
                                (this._currentState.position = m),
                                (this._currentState.velocity = v),
                                this._timeAccumulator > 0 &&
                                    this._interpolate(this._timeAccumulator / o.SOLVER_TIMESTEP_SEC),
                                (this.isAtRest() || (this._overshootClampingEnabled && this.isOvershooting())) &&
                                    (this._springConfig.tension > 0
                                        ? ((this._startValue = this._endValue),
                                          (this._currentState.position = this._endValue))
                                        : ((this._endValue = this._currentState.position),
                                          (this._startValue = this._endValue)),
                                    this.setVelocity(0),
                                    (i = !0));
                            let $ = !1;
                            this._wasAtRest && ((this._wasAtRest = !1), ($ = !0));
                            let S = !1;
                            i && ((this._wasAtRest = !0), (S = !0)), this.notifyPositionUpdated($, S);
                        },
                        notifyPositionUpdated: function (t, e) {
                            for (let i = 0, s = this.listeners.length; i < s; i++) {
                                let s = this.listeners[i];
                                t && s.onSpringActivate && s.onSpringActivate(this),
                                    s.onSpringUpdate && s.onSpringUpdate(this),
                                    e && s.onSpringAtRest && s.onSpringAtRest(this);
                            }
                        },
                        systemShouldAdvance: function () {
                            return !this.isAtRest() || !this.wasAtRest();
                        },
                        wasAtRest: function () {
                            return this._wasAtRest;
                        },
                        isAtRest: function () {
                            return (
                                Math.abs(this._currentState.velocity) < this._restSpeedThreshold &&
                                (this.getDisplacementDistanceForState(this._currentState) <=
                                    this._displacementFromRestThreshold ||
                                    0 === this._springConfig.tension)
                            );
                        },
                        setAtRest: function () {
                            return (
                                (this._endValue = this._currentState.position),
                                (this._tempState.position = this._currentState.position),
                                (this._currentState.velocity = 0),
                                this
                            );
                        },
                        _interpolate: function (t) {
                            (this._currentState.position =
                                this._currentState.position * t + this._previousState.position * (1 - t)),
                                (this._currentState.velocity =
                                    this._currentState.velocity * t + this._previousState.velocity * (1 - t));
                        },
                        getListeners: function () {
                            return this.listeners;
                        },
                        addListener: function (t) {
                            return this.listeners.push(t), this;
                        },
                        removeListener: function (t) {
                            return p(this.listeners, t), this;
                        },
                        removeAllListeners: function () {
                            return (this.listeners = []), this;
                        },
                    });
                let a = function () {};
                t.extend(a.prototype, { position: 0, velocity: 0 });
                let r = (s.SpringConfig = function (t, e) {
                        (this.tension = t), (this.friction = e);
                    }),
                    l = (s.AnimationLooper = function () {
                        this.springSystem = null;
                        let e = this,
                            i = function () {
                                e.springSystem.loop(Date.now());
                            };
                        this.run = function () {
                            t.onFrame(i);
                        };
                    });
                (s.SimulationLooper = function (t) {
                    this.springSystem = null;
                    let e = 0,
                        i = !1;
                    (t = t || 16.667),
                        (this.run = function () {
                            if (!i) {
                                for (i = !0; !this.springSystem.getIsIdle(); ) this.springSystem.loop((e += t));
                                i = !1;
                            }
                        });
                }),
                    (s.SteppingSimulationLooper = function (t) {
                        this.springSystem = null;
                        let e = 0;
                        (this.run = function () {}),
                            (this.step = function (t) {
                                this.springSystem.loop((e += t));
                            });
                    });
                let c = (s.OrigamiValueConverter = {
                        tensionFromOrigamiValue: function (t) {
                            return 3.62 * (t - 30) + 194;
                        },
                        frictionFromOrigamiValue: function (t) {
                            return 3 * (t - 8) + 25;
                        },
                    }),
                    d = (s.BouncyConversion = function (t, e) {
                        this.speed = e;
                        var i = this.normalize(t / 1.7, 0, 20);
                        i = this.projectNormal(i, 0, 0.8);
                        var s = this.normalize(e / 1.7, 0, 20);
                        (this.bouncyTension = this.projectNormal(s, 0.5, 200)),
                            (this.bouncyFriction = this.quadraticOutInterpolation(
                                i,
                                this.b3Nobounce(this.bouncyTension),
                                0.01
                            ));
                    });
                t.extend(d.prototype, {
                    normalize: function (t, e, i) {
                        return (t - e) / (i - e);
                    },
                    projectNormal: function (t, e, i) {
                        return e + t * (i - e);
                    },
                    linearInterpolation: function (t, e, i) {
                        return t * i + (1 - t) * e;
                    },
                    quadraticOutInterpolation: function (t, e, i) {
                        return this.linearInterpolation(2 * t - t * t, e, i);
                    },
                    b3Friction1: function (t) {
                        return 7e-4 * Math.pow(t, 3) - 0.031 * Math.pow(t, 2) + 0.64 * t + 1.28;
                    },
                    b3Friction2: function (t) {
                        return 44e-6 * Math.pow(t, 3) - 0.006 * Math.pow(t, 2) + 0.36 * t + 2;
                    },
                    b3Friction3: function (t) {
                        return 45e-8 * Math.pow(t, 3) - 332e-6 * Math.pow(t, 2) + 0.1078 * t + 5.84;
                    },
                    b3Nobounce: function (t) {
                        let e = 0;
                        return (
                            (e =
                                t <= 18
                                    ? this.b3Friction1(t)
                                    : t > 18 && t <= 44
                                    ? this.b3Friction2(t)
                                    : this.b3Friction3(t)),
                            e
                        );
                    },
                }),
                    t.extend(r, {
                        fromOrigamiTensionAndFriction: function (t, e) {
                            return new r(c.tensionFromOrigamiValue(t), c.frictionFromOrigamiValue(e));
                        },
                        fromBouncinessAndSpeed: function (t, e) {
                            var i = new s.BouncyConversion(t, e);
                            return this.fromOrigamiTensionAndFriction(i.bouncyTension, i.bouncyFriction);
                        },
                    }),
                    (r.DEFAULT_ORIGAMI_SPRING_CONFIG = r.fromOrigamiTensionAndFriction(40, 7)),
                    t.extend(r.prototype, { friction: 0, tension: 0 });
                let h = {};
                (t.hexToRGB = function (t) {
                    if (h[t]) return h[t];
                    3 === (t = t.replace('#', '')).length && (t = t[0] + t[0] + t[1] + t[1] + t[2] + t[2]);
                    let e = t.match(/.{2}/g),
                        i = { r: parseInt(e[0], 16), g: parseInt(e[1], 16), b: parseInt(e[2], 16) };
                    return (h[t] = i), i;
                }),
                    (t.rgbToHex = function (t, e, i) {
                        return (
                            (t = t.toString(16)),
                            (e = e.toString(16)),
                            (i = i.toString(16)),
                            '#' +
                                (t = t.length < 2 ? '0' + t : t) +
                                (e = e.length < 2 ? '0' + e : e) +
                                (i = i.length < 2 ? '0' + i : i)
                        );
                    });
                let u,
                    _ = (s.MathUtil = {
                        mapValueInRange: function (t, e, i, s, n) {
                            return s + ((t - e) / (i - e)) * (n - s);
                        },
                        degreesToRadians: function (t) {
                            return (t * Math.PI) / 180;
                        },
                    });
                function p(t, e) {
                    let i = t.indexOf(e);
                    -1 != i && t.splice(i, 1);
                }
                t.extend(t, _),
                    'undefined' != typeof window &&
                        (u =
                            window.requestAnimationFrame ||
                            window.webkitRequestAnimationFrame ||
                            window.mozRequestAnimationFrame ||
                            window.msRequestAnimationFrame ||
                            window.oRequestAnimationFrame ||
                            function (t) {
                                window.setTimeout(t, 1e3 / 60);
                            }),
                    u || 'undefined' == typeof process || 'node' !== process.title || (u = setImmediate),
                    (t.onFrame = function (t) {
                        return u(t);
                    }),
                    'undefined' != typeof exports
                        ? t.extend(exports, s)
                        : 'undefined' != typeof window && (window.rebound = s);
            })();
            var n = (function () {
                function t(t, e) {
                    for (let i = 0; i < e.length; i++) {
                        let s = e[i];
                        (s.enumerable = s.enumerable || !1),
                            (s.configurable = !0),
                            'value' in s && (s.writable = !0),
                            Object.defineProperty(t, s.key, s);
                    }
                }
                return function (e, i, s) {
                    return i && t(e.prototype, i), s && t(e, s), e;
                };
            })();
            let o = (function () {
                function t() {
                    let e = arguments.length <= 0 || void 0 === arguments[0] ? 100 : arguments[0],
                        i = arguments.length <= 1 || void 0 === arguments[1] ? 3 : arguments[1],
                        s = arguments.length <= 2 || void 0 === arguments[2] ? 0 : arguments[2],
                        n = arguments[3];
                    a(this, t),
                        (this._radius = e),
                        (this._sides = i),
                        (this._depth = s),
                        (this._colors = n),
                        (this._x = 0),
                        (this._y = 0),
                        (this.rotation = 0),
                        (this.scale = 1),
                        (this.points = this._getRegularPolygonPoints());
                }
                return (
                    n(t, [
                        {
                            key: '_getRegularPolygonPoints',
                            value: function () {
                                let t = [],
                                    e = 0;
                                for (; e < this._sides; ) {
                                    let i = -this._radius * Math.sin((2 * e * Math.PI) / this._sides),
                                        s = this._radius * Math.cos((2 * e * Math.PI) / this._sides);
                                    t.push({ x: i, y: s }), e++;
                                }
                                return t;
                            },
                        },
                        {
                            key: '_getInscribedPoints',
                            value: function (t, e) {
                                let i = this,
                                    s = [];
                                return (
                                    t.forEach(function (n, o) {
                                        let a = n,
                                            r = t[o + 1];
                                        r || (r = t[0]);
                                        let l = i._getInterpolatedPoint(a, r, e);
                                        s.push(l);
                                    }),
                                    s
                                );
                            },
                        },
                        {
                            key: '_getInterpolatedPoint',
                            value: function (t, e, i) {
                                let s = t.x,
                                    n = t.y;
                                return { x: s + (e.x - s) * i, y: n + (e.y - n) * i };
                            },
                        },
                        {
                            key: '_getUpdatedChildren',
                            value: function (t) {
                                let e = [];
                                for (let i = 0; i < this._depth; i++) {
                                    let s = e[i - 1] || this.points,
                                        n = this._getInscribedPoints(s, t);
                                    e.push(n);
                                }
                                return e;
                            },
                        },
                        {
                            key: 'renderChildren',
                            value: function (t, e) {
                                let i = this,
                                    n = this._getUpdatedChildren(e);
                                n.forEach(function (e, o) {
                                    t.beginPath(),
                                        e.forEach(function (e) {
                                            return t.lineTo(e.x, e.y);
                                        }),
                                        t.closePath();
                                    let a = i._colors.stroke,
                                        r = i._colors.child;
                                    if ((a && ((t.strokeStyle = a), t.stroke()), r)) {
                                        let e = s.util.hexToRGB(r),
                                            i = 1 / n.length,
                                            a = i + i * o,
                                            l = 'rgba(' + e.r + ', ' + e.g + ', ' + e.b + ', ' + a + ')';
                                        (t.fillStyle = l),
                                            (t.shadowColor = 'rgba(0,0,0, 0.1)'),
                                            (t.shadowBlur = 10),
                                            (t.shadowOffsetX = 0),
                                            (t.shadowOffsetY = 0),
                                            t.fill();
                                    }
                                });
                            },
                        },
                        {
                            key: 'render',
                            value: function (t) {
                                t.save(),
                                    t.translate(this._x, this._y),
                                    0 !== this.rotation && t.rotate(s.MathUtil.degreesToRadians(this.rotation)),
                                    1 !== this.scale && t.scale(this.scale, this.scale),
                                    t.beginPath(),
                                    this.points.forEach(function (e) {
                                        return t.lineTo(e.x, e.y);
                                    }),
                                    t.closePath();
                                let e = this._colors.stroke,
                                    i = this._colors.base;
                                e && ((t.strokeStyle = e), t.stroke()), i && ((t.fillStyle = i), t.fill()), t.restore();
                            },
                        },
                    ]),
                    t
                );
            })();
            n = (function () {
                function t(t, e) {
                    for (let i = 0; i < e.length; i++) {
                        let s = e[i];
                        (s.enumerable = s.enumerable || !1),
                            (s.configurable = !0),
                            'value' in s && (s.writable = !0),
                            Object.defineProperty(t, s.key, s);
                    }
                }
                return function (e, i, s) {
                    return i && t(e.prototype, i), s && t(e, s), e;
                };
            })();
            function a(t, e) {
                if (!(t instanceof e)) throw new TypeError('Cannot call a class as a function');
            }
            let r = (function () {
                function t(e) {
                    a(this, t);
                    let i = e.radius,
                        s = e.sides,
                        n = e.depth,
                        r = e.colors,
                        l = e.alwaysForward,
                        c = e.restAt,
                        d = e.renderBase;
                    s < 3 && (console.warn('At least 3 sides required.'), (s = 3)),
                        (this._canvas = document.createElement('canvas')),
                        (this._canvas.style.backgroundColor = r.background),
                        (this._canvas.style.zIndex = 1100),
                        (this._canvasW = null),
                        (this._canvasH = null),
                        (this._canvasOpacity = 1),
                        (this._centerX = null),
                        (this._centerY = null),
                        (this._alwaysForward = l),
                        (this._restThreshold = c),
                        (this._renderBase = d),
                        (this._springRangeLow = 0),
                        (this._springRangeHigh = this._restThreshold || 1),
                        (this._basePolygon = new o(i, s, n, r)),
                        (this._progress = 0),
                        (this._isAutoSpin = null),
                        (this._isCompleting = null);
                }
                return (
                    n(t, [
                        {
                            key: 'init',
                            value: function (t, e) {
                                this._addCanvas(),
                                    (this._spring = t),
                                    this._addSpringListener(),
                                    (this._isAutoSpin = e),
                                    e ? this._spin() : (this._spring.setEndValue(0), this.render());
                            },
                        },
                        {
                            key: '_addCanvas',
                            value: function () {
                                document.body.appendChild(this._canvas),
                                    (this._context = this._canvas.getContext('2d')),
                                    this._setCanvasSize();
                            },
                        },
                        {
                            key: '_setCanvasSize',
                            value: function () {
                                (this._canvasW = this._canvas.width = window.innerWidth),
                                    (this._canvasH = this._canvas.height = window.innerHeight),
                                    (this._canvas.style.position = 'fixed'),
                                    (this._canvas.style.top = 0),
                                    (this._canvas.style.left = 0),
                                    (this._centerX = this._canvasW / 2),
                                    (this._centerY = this._canvasH / 2);
                            },
                        },
                        {
                            key: '_addSpringListener',
                            value: function () {
                                let t = this;
                                this._spring.addListener({
                                    onSpringUpdate: function (e) {
                                        let i = e.getCurrentValue(),
                                            n = t._springRangeLow,
                                            o = t._springRangeHigh;
                                        (i = s.MathUtil.mapValueInRange(i, 0, 1, n, o)), t.render(i);
                                    },
                                });
                            },
                        },
                        {
                            key: 'setComplete',
                            value: function () {
                                this._isCompleting = !0;
                            },
                        },
                        {
                            key: '_completeAnimation',
                            value: function () {
                                (this._canvasOpacity -= 0.1),
                                    (this._canvas.style.opacity = this._canvasOpacity),
                                    this._canvasOpacity <= 0 &&
                                        ((this._isAutoSpin = !1), this._spring.setAtRest(), this._canvas.remove());
                            },
                        },
                        {
                            key: '_spin',
                            value: function () {
                                if (this._alwaysForward) {
                                    let t = this._spring.getCurrentValue();
                                    this._restThreshold && 1 === t && this._switchSpringRange(),
                                        1 === t && this._spring.setCurrentValue(0).setAtRest();
                                }
                                this._spring.setEndValue(1 === this._spring.getCurrentValue() ? 0 : 1);
                            },
                        },
                        {
                            key: '_switchSpringRange',
                            value: function () {
                                let t = this._restThreshold;
                                (this._springRangeLow = this._springRangeLow === t ? 0 : t),
                                    (this._springRangeHigh = this._springRangeHigh === t ? 1 : t);
                            },
                        },
                        {
                            key: 'render',
                            value: function (t) {
                                t && (this._progress = Math.round(1e4 * t) / 1e4),
                                    this._isAutoSpin && this._spring.isAtRest() && this._spin(),
                                    this._isCompleting && this._completeAnimation(),
                                    this._context.clearRect(0, 0, this._canvasW, this._canvasH),
                                    this._context.save(),
                                    this._context.translate(this._centerX, this._centerY),
                                    (this._context.lineWidth = 1.5),
                                    this._renderBase && this._basePolygon.render(this._context),
                                    this._basePolygon.renderChildren(this._context, this._progress),
                                    this._context.restore();
                            },
                        },
                    ]),
                    t
                );
            })();
            function l() {
                return new (function () {
                    let t = this;
                    (this.config = $.__config.loading),
                        (this.spring = null),
                        (this.spinner = null),
                        (this.initRebound = () => {
                            let e = t.config.rebound,
                                i = new s.SpringSystem();
                            t.spring = i.createSpring(e.tension, e.friction);
                        }),
                        (this.initSpinner = () => {
                            let e = t.config.spinner;
                            t.spinner = new r(e);
                        }),
                        (this.start = () => {
                            'function' == typeof $.__config.hooks.beforeLoading && $.__config.hooks.beforeLoading(),
                                $('#blog-news').prepend('<div id="loading"></div>'),
                                t.initRebound(),
                                t.initSpinner(),
                                t.spinner.init(t.spring, !0);
                        }),
                        (this.stop = () => {
                            $('body').css('overflow', 'auto'),
                                t.spinner.setComplete(),
                                $('div#loading').hide(),
                                $('a[name="top"]').hide(),
                                'function' == typeof $.__config.hooks.afterLoading && $.__config.hooks.afterLoading();
                        });
                })();
            }
        },
        8789: function (t, e, i) {
            function s() {
                function t(t) {
                    return new RegExp('(^|\\s+)' + t + '(\\s+|$)');
                }
                let e, i, s;
                function n(t, n) {
                    (e(t, n) ? s : i)(t, n);
                }
                return (
                    'classList' in document.documentElement
                        ? ((e = function (t, e) {
                              return t.classList.contains(e);
                          }),
                          (i = function (t, e) {
                              t.classList.add(e);
                          }),
                          (s = function (t, e) {
                              t.classList.remove(e);
                          }))
                        : ((e = function (e, i) {
                              return t(i).test(e.className);
                          }),
                          (i = function (t, i) {
                              e(t, i) || (t.className = t.className + ' ' + i);
                          }),
                          (s = function (e, i) {
                              e.className = e.className.replace(t(i), ' ');
                          })),
                    { hasClass: e, addClass: i, removeClass: s, toggleClass: n, has: e, add: i, remove: s, toggle: n }
                );
            }
            i.d(e, {
                Z: function () {
                    return s;
                },
            });
        },
        9370: function (t, e, i) {
            i.a(
                t,
                async function (t, s) {
                    try {
                        i.d(e, {
                            Z: function () {
                                return o;
                            },
                        });
                        var n = i(8789);
                        function o() {
                            let t,
                                e = document.body,
                                i = document.querySelector('.content-wrap'),
                                s = document.getElementById('open-button'),
                                o = document.getElementById('close-button'),
                                a = !1,
                                r = (0, n.Z)(),
                                l = document.getElementById('morph-shape'),
                                c = Snap(l.querySelector('svg')).select('path'),
                                d = c.attr('d'),
                                h = !1;
                            function u() {
                                $('.menu-wrap').show(),
                                    a
                                        ? ($(e).removeClass('show-menu'),
                                          $('#content-wrap').fadeOut(300),
                                          $(e).css('overflow', 'auto'),
                                          $('#mainContent').off('touchmove'),
                                          c.attr('d', d),
                                          (h = !1))
                                        : (r.add(e, 'show-menu'),
                                          $('#content-wrap').show(),
                                          $('body').css('overflow', 'hidden'),
                                          t.scrollTo(!1, 'top')),
                                    (a = !a);
                            }
                            return (
                                s.addEventListener('click', u),
                                o && o.addEventListener('click', u),
                                i.addEventListener('click', function (t) {
                                    let e = t.target;
                                    a && e !== s && u();
                                }),
                                (t = new Optiscroll(document.querySelector('#menuWrap'), {
                                    preventParentScroll: !0,
                                    forceScrollbars: !0,
                                })),
                                { myOptiscrollInstance: t }
                            );
                        }
                        await $.__tools
                            .dynamicLoadingJs($.__config.default.snapsvg)
                            .catch((t) => console.error('snapsvg-cjs.js', t)),
                            await $.__tools
                                .dynamicLoadingJs($.__config.default.optiscroll)
                                .catch((t) => console.log('optiscroll.js', t)),
                            await $.__tools.dynamicLoadingCss($.__config.default.optiscrollcss),
                            s();
                    } catch (a) {
                        s(a);
                    }
                },
                1
            );
        },
        9387: function (t, e, i) {
            i.a(t, async function (t, s) {
                try {
                    i.d(e, {
                        Z: function () {
                            return l;
                        },
                    });
                    var n = i(4791),
                        o = i(7996),
                        a = i(9370),
                        r = t([a]);
                    function l() {
                        let t;
                        $('#sidebar_news').prepend(n.Z),
                            (t = (0, a.Z)()),
                            (() => {
                                $('.sidebar-footer').html(o.Z),
                                    $('#nav_host').attr('href', $.__status.homeUrl),
                                    $('#nav_connect').attr('href', $.__config.info.connect),
                                    $('#nav_rss').attr('href', $.__status.homeUrl + '/index.php/feed/'),
                                    $('#nav_manage').attr('href', $.__status.homeUrl + '/admin');
                                let t = $.__config.info.avatar ? $.__config.info.avatar : $.__config.default.avatar;
                                $('#menuBlogAvatar').append(
                                    "<img class='img-responsive' alt='用户头像' src='" + t + "'>"
                                ),
                                    $('.sidebar-title-msg').text($.__config.sidebar.titleMsg);
                            })(),
                            (() => {
                                let t = $.__config.sidebar.infoBackground
                                    ? $.__config.sidebar.infoBackground
                                    : 'https://www.oyo.cool/usr/uploads/imgs/o_221114135508_sidebar_bg_2.webp';
                                $('.container .menu-wrap').css('background-image', "url('" + t + "')");
                            })(),
                            (() => {
                                let t = 1e3;
                                function e(t, e, i) {
                                    t.length > 0 &&
                                        '' === e.html() &&
                                        (e.html(
                                            (function (t) {
                                                let e = '<ul>',
                                                    i = /^[1-9]+[\d]*$/;
                                                return (
                                                    t.each((s) => {
                                                        let n = $(t[s]),
                                                            o = n.text() === n.html() ? {} : $(n.html()),
                                                            a = n.text().trim().split('.');
                                                        i.test(a[0]) && a.splice(0, 1);
                                                        let r = a.join('.').trim();
                                                        o.length > 0 && o.html(r),
                                                            r.length > 0 &&
                                                                (e +=
                                                                    '<li>' +
                                                                    (o.length > 0
                                                                        ? o.prop('outerHTML')
                                                                        : '<a href="javascript:void(0);">' +
                                                                          r +
                                                                          '</a>') +
                                                                    '</li>');
                                                    }),
                                                    (e += '</ul>'),
                                                    e
                                                );
                                            })(t)
                                        ),
                                        e.parent('.sidebar-dropdown').show(),
                                        $.__tools.clearIntervalTimeId(i));
                                }
                                ($.__timeIds.introduceTId = window.setInterval(() => {
                                    $('#info_name').text($.__config.info.name),
                                        $('#info_job').text($.__config.info.job),
                                        $('#info_position').text($.__config.info.position),
                                        $('#info_proverb').text($.__config.info.proverb);
                                    let t = $('#profile_block').html(),
                                        e = $('#introduce');
                                    'string' == typeof t &&
                                        '' === e.html() &&
                                        (e.html($.__tools.htmlFiltrationScript(t)),
                                        $.__tools.clearIntervalTimeId($.__timeIds.introduceTId));
                                }, t)),
                                    $.__config.sidebar.blogStatus &&
                                        ($.__timeIds.blogStatsTId = window.setInterval(() => {
                                            let t = $('.blogStats'),
                                                e = $('.sidebar-stats');
                                            t.length > 0 &&
                                                (e.html($.__tools.htmlFiltrationScript(t.html())).show(),
                                                t.html(''),
                                                $.__tools.clearIntervalTimeId($.__timeIds.blogStatsTId));
                                        }, t)),
                                    ($.__timeIds.calendarTId = window.setInterval(() => {
                                        let t = $('#blogCalendar'),
                                            e = $('#blog-calendar'),
                                            i = $('#calendar-box');
                                        if (t.length > 0 && '' === i.html()) {
                                            let t = '<div id="blog-calendar">' + e.html() + '</div>';
                                            e.remove(),
                                                i.html(t).show(),
                                                $('#blog-calendar').css('visibility', 'visible'),
                                                $.__tools.clearIntervalTimeId($.__timeIds.calendarTId);
                                        }
                                    }, t)),
                                    ($.__timeIds.searchTId = window.setInterval(() => {
                                        let t = $('#sidebar_search_box'),
                                            e = $('#sb-sidebarSearchBox');
                                        t.length > 0 &&
                                            '' === e.html() &&
                                            (e.prepend(t),
                                            $('.sidebar-search').show(),
                                            $.__tools.clearIntervalTimeId($.__timeIds.searchTId));
                                    }, t)),
                                    ($.__timeIds.newEssayTId = window.setInterval(() => {
                                        e(
                                            $('#sidebar_recentposts ul li'),
                                            $('#sb-sidebarRecentposts'),
                                            $.__timeIds.newEssayTId
                                        );
                                    }, t)),
                                    ($.__timeIds.topTagsTId = window.setInterval(() => {
                                        e($('#sidebar_toptags ul li'), $('#sb-toptags'), $.__timeIds.topTagsTId);
                                    }, t)),
                                    ($.__timeIds.classifyTId = window.setInterval(() => {
                                        e($('#sidebar_postcategory ul li'), $('#sb-classify'), $.__timeIds.classifyTId);
                                    }, t)),
                                    ($.__timeIds.recordTId = window.setInterval(() => {
                                        e(
                                            $('#sidebar_postarchive ul li'),
                                            $('#sb-record'),
                                            'icon-task-fill',
                                            $.__timeIds.recordTId
                                        );
                                    }, t)),
                                    ($.__timeIds.topViewTId = window.setInterval(() => {
                                        e($('#TopViewPostsBlock ul li'), $('#sb-topview'), $.__timeIds.topViewTId);
                                    }, t)),
                                    ($.__timeIds.topDiggPostsTId = window.setInterval(() => {
                                        e(
                                            $('#TopDiggPostsBlock ul li'),
                                            $('#sb-topDiggPosts'),
                                            $.__timeIds.topDiggPostsTId
                                        );
                                    }, t)),
                                    ($.__timeIds.commentsTId = window.setInterval(() => {
                                        let t = $('#sidebar_recentcomments ul'),
                                            e = $('#sb-recentComments');
                                        t.length > 0 &&
                                            '' === e.html() &&
                                            (e.html(
                                                ((t) => {
                                                    let e,
                                                        i,
                                                        s,
                                                        n = '<ul>',
                                                        o = /^[1-9]+[\d]*$/;
                                                    if (t.find('li').length > 2) {
                                                        if (
                                                            ((e = t.find('li.recent_comment_title')),
                                                            (i = t.find('li.recent_comment_body')),
                                                            (s = t.find('li.recent_comment_author')),
                                                            e.length !== i.length || e.length !== s.length)
                                                        )
                                                            return;
                                                        e.each((t) => {
                                                            let a = $(e[t]),
                                                                r = a.text() === a.html() ? {} : $(a.html()),
                                                                l = a.text().trim().split('.');
                                                            o.test(l[0]) && l.splice(0, 1);
                                                            let c = l.join('.').trim();
                                                            r.length > 0 && r.html(c),
                                                                (n +=
                                                                    '<li>' +
                                                                    (r.length > 0
                                                                        ? r.prop('outerHTML')
                                                                        : "<a href='javascript:void(0);'>" +
                                                                          c +
                                                                          '</a>') +
                                                                    '<div class="sb-recent_comment_body">' +
                                                                    $(i[t]).text() +
                                                                    '</div><div class="sb-recent_comment_author">' +
                                                                    $(s[t]).text() +
                                                                    '</div></li>');
                                                        });
                                                    }
                                                    return (n += '</ul>'), n;
                                                })(t)
                                            ),
                                            e.parent('.sidebar-dropdown').show(),
                                            $.__tools.clearIntervalTimeId($.__timeIds.commentsTId));
                                    }, t)),
                                    ($.__timeIds.commentsRankTId = window.setInterval(() => {
                                        e(
                                            $('#TopFeedbackPostsBlock ul li'),
                                            $('#sb-commentsRank'),
                                            $.__timeIds.commentsRankTId
                                        );
                                    }, t)),
                                    (() => {
                                        let t = $.__config.sidebar.navList,
                                            e = '';
                                        t.length > 0 &&
                                            ((e = '<ul>'),
                                            $.each(t, function (i) {
                                                let s = t[i].length > 2 ? t[i][2] : 'icon-qianzishenhe';
                                                e +=
                                                    '<li><a href="' +
                                                    t[i][1] +
                                                    '" class="sidebar-dropdown-box" target="_blank"><i class="iconfont ' +
                                                    s +
                                                    '"></i>' +
                                                    t[i][0] +
                                                    '</a></li>';
                                            }),
                                            (e += '</ul>'),
                                            $('.customize-nav').append(e).show());
                                    })(),
                                    (() => {
                                        let t = $.__config.sidebar.customList;
                                        if (Object.keys(t).length > 0) {
                                            let e = '';
                                            $.each(t, (t, i) => {
                                                let s = '<li class="ng-star-inserted sidebar-dropdown">';
                                                (s +=
                                                    '<a href="javascript:void(0)" class="ng-star-inserted sidebar-dropdown-box">'),
                                                    (s += '   <i class="iconfont ' + i.icon + '"></i>'),
                                                    (s += '   <span class="sidebar-dropdown-title">' + t + '</span>'),
                                                    (s += '</a>'),
                                                    (s += '<div class="sidebar-submenu"><ul>'),
                                                    $.each(i.data, (t, e) => {
                                                        s +=
                                                            '<li><a href="' +
                                                            e[1] +
                                                            '" target="_blank">' +
                                                            e[0] +
                                                            '</a></li>';
                                                    }),
                                                    (s += '</ul></div>'),
                                                    (s += '</li>'),
                                                    (e += s);
                                            }),
                                                $('#customize-sidebar-menu ul').append(e),
                                                $('#customize-sidebar-menu').show(),
                                                $('#customize-sidebar-menu .sidebar-dropdown').show();
                                        }
                                    })();
                            })(),
                            $.__config.animate.avatar.enable &&
                                ($('#menuBlogAvatar').addClass('img-rounded'),
                                $('.author_avatar').addClass('img-rounded')),
                            $.__config.sidebar.submenu.latestPosts &&
                                ($('#sb-sidebarRecentposts').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-sidebarRecentposts').show()),
                            $.__config.sidebar.submenu.myTags &&
                                ($('#sb-toptags').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-toptags').show()),
                            $.__config.sidebar.submenu.postsClassify &&
                                ($('#sb-classify').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-classify').show()),
                            $.__config.sidebar.submenu.readRank &&
                                ($('#sb-topview').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-topview').show()),
                            $.__config.sidebar.submenu.recommendRank &&
                                ($('#sb-topDiggPosts').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-topDiggPosts').show()),
                            $.__config.sidebar.submenu.latestComment &&
                                ($('#sb-recentComments').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-recentComments').show()),
                            $.__config.sidebar.submenu.commentsRank &&
                                ($('#sb-commentsRank').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-commentsRank').show()),
                            $.__config.sidebar.submenu.postsArchive &&
                                ($('#sb-record').parent('li.sidebar-dropdown').addClass('active'),
                                $('#sb-record').show()),
                            $.__config.sidebar.submenu.customList &&
                                ($('#customize-sidebar-menu .sidebar-submenu')
                                    .parent('li.sidebar-dropdown')
                                    .addClass('active'),
                                $('#customize-sidebar-menu .sidebar-submenu').show()),
                            $('.sidebar-menu a.sidebar-dropdown-box').on('click', function () {
                                let e = $(this).parent('li.sidebar-dropdown'),
                                    i = e.find('.sidebar-submenu');
                                i.length > 0 &&
                                    (e.hasClass('active')
                                        ? (e.removeClass('active'), i.slideUp(300))
                                        : (e.addClass('active'), i.slideDown(300)),
                                    setTimeout(function () {
                                        t && void 0 !== t.myOptiscrollInstance && t.myOptiscrollInstance.update();
                                    }, 300));
                            }),
                            $.__event.resize.handle.push(() => {
                                setTimeout(function () {
                                    $('body').hasClass('show-menu') &&
                                        t &&
                                        void 0 !== t.myOptiscrollInstance &&
                                        t.myOptiscrollInstance.update();
                                }, 300);
                            });
                    }
                    (a = (r.then ? (await r)() : r)[0]), s();
                } catch (c) {
                    s(c);
                }
            });
        },
    },
]);
